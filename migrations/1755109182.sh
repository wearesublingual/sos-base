...
# Users can still opt-in to Cloudflare DNS using: sublingual-os-setup-dns cloudflare
...
if [[ -f /etc/systemd/resolved.conf ]]; then
  ...
  if [[ -f /etc/systemd/network/99-sublingual-os-dns.network ]]; then
    sudo rm -f /etc/systemd/network/99-sublingual-os-dns.network
  ...
  echo "To use Cloudflare DNS, run: sublingual-os-setup-dns Cloudflare"
fi