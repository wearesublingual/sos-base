...
show_log_tail() {
  if [[ -f $SUBLINGUAL_OS_INSTALL_LOG_FILE ]]; then
    local log_lines=$((TERM_HEIGHT - LOGO_HEIGHT - 35))
    local max_line_width=$((LOGO_WIDTH - 4))

    tail -n $log_lines "$SUBLINGUAL_OS_INSTALL_LOG_FILE" | while IFS= read -r line; do
...
catch_errors() {
  ...
  while true; do
    ...
    if [[ -n ${SUBLINGUAL_OS_ONLINE_INSTALL:-} ]]; then
    ...
    case "$choice" in
    "Retry installation")
      bash ~/.local/share/sublingual-os/install.sh
      break
      ;;
    "View full log")
      if command -v less &>/dev/null; then
        less "$SUBLINGUAL_OS_INSTALL_LOG_FILE"
      else
        tail "$SUBLINGUAL_OS_INSTALL_LOG_FILE"
      fi
      ;;
